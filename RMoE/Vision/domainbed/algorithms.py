...
class GMOE(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.min_aux_loss_weight = hparams.get('min_aux_loss_weight', 0.0)
        self.max_aux_loss_weight = hparams.get('max_aux_loss_weight', 1.0)
        self.dw = hparams.get('dw', False)
        if self.dw:
            self.aux_loss_weight = self.max_aux_loss_weight
    ...
    def update(self, minibatches, unlabeled=None):
        if self.dw:
            self.aux_loss_weight = max(self.min_aux_loss_weight, self.aux_loss_weight * self.hparams['dw_momentum'])
