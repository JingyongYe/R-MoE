    ...
    for i, layer_module in enumerate(model.vit.encoder.layer):
        if i in args.moe_layers:
            ...
            if not args.MoE_from_ffn:
                ...
                if hasattr(args, 'dw') and args.dw:
                    moe_kwargs['dw'] = True
                    moe_kwargs['dw_momentum'] = getattr(args, 'dw_momentum', 0.99)
                if args.router == 'gated_multi_gate':
                    layer_module.mlp = tutel_moe.moe_layer(**moe_kwargs)
                else:
                    moe_kwargs['gate_type'] = {'type': args.router, 'k': args.topk, 'fp32_gate': True, 
                            'gate_noise': args.gate_noise,  #kwargs.get('gate_noise', 1.0), 
                            'capacity_factor': args.capacity_factor}
                    moe_kwargs['is_gshard_loss'] = False
                    layer_module.mlp = tutel_moe.moe_layer(**moe_kwargs)
