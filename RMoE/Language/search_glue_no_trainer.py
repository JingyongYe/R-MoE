...
def parse_args():
    ...
    # New arguments for Decoupled Gating and Dynamic Auxiliary Loss
    parser.add_argument('--dw', action='store_true', help='Enable Decoupled Gating with momentum-based fusion weights.')
    parser.add_argument('--dw_momentum', type=float, default=0.99, help='Momentum for updating fusion weights.')
    parser.add_argument('--min_aux_loss_weight', type=float, default=0.001, help='Minimum auxiliary loss weight at the start of training.')
    parser.add_argument('--max_aux_loss_weight', type=float, default=0.01, help='Maximum auxiliary loss weight at the end of training.')
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            if args.to_MoE and  args.aux_loss_weight > 0:
                # Dynamic auxiliary loss weight calculation
                if args.dw:
                    progress = min(completed_steps / args.max_train_steps, 1.0)
                    current_aux_loss_weight = args.min_aux_loss_weight + (args.max_aux_loss_weight - args.min_aux_loss_weight) * progress
                else:
                    current_aux_loss_weight = args.aux_loss_weight
                
                #for ablation study
                #current_aux_loss_weight=1
...
if __name__ == "__main__":
    ...
    if args.save_model:
        args.output_dir = os.path.join('./results_dw_nolossmodify', args.model_name_or_path+"_save", args.task_name + (f'/{args.output_dir}' if args.output_dir else ''))
    else:
        args.output_dir = os.path.join('./results_dw', args.model_name_or_path, args.task_name + (f'/{args.output_dir}' if args.output_dir else ''))
